...
<x-layout>
  <div class="container">
    <div class="homescreen">
      ...
      <ul class="homescreen__categories">
        @foreach($categories as $category)
          <li>
              <input data-url="{{ route('products.getcategory') }}" class="check-cate" name="check-cate" type="checkbox" value="{{ $category->id }}">&nbsp{{ $category->category_name }}</input>
          </li>
        @endforeach
      </ul>
      <div class="homescreen__product">
           @foreach($products as $product)
        <div class="product"><img class="card-img-top product-photo" src="{{ asset('images/' . $product->product_photo) }}" alt="..." data-product-id="{{ $product->id }}"/>
